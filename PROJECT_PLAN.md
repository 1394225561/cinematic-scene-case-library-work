...
- 每个精华案例包含来源、适用条件、原始案例、模型无关骨架、变量槽位、适配说明和质量检查。
...
1. 为 `cinema-studio-production` 准备最小案例检索路由补丁。
2. 验证案例库不会取代 LIRA、ACTING、CINEDANCE 的职责。
3. 验证 H3 适配是否只由案例库提供参考，还是需要进一步调整总编排契约。
4. 运行 Skill 结构校验、索引链接校验、案例 ID 校验和无重资产检查。
5. 使用代表性任务测试检索与下游交接。

注意：当前 `minimax-h3-director` 明确禁止作为 `cinema-studio-production` 的下游包装器。是否扩展统一入口属于后续独立架构决策，不能在本阶段前擅自修改。
...
1. 等待用户审核 4B-5 全量规范化检查点、6555 条哈希映射、61 条人工复核入口、问题清单和阶段 5 建议。
2. 若审核要求修改，只调整指出的检查点或规范化规则并重跑受影响记录；不得绕过 4B-5 直接进入阶段 5。
3. 用户明确批准 4B-5 后，才开始阶段 5 的场景分类与质量评分；在此之前不生成全量 Seedance/H3 最终稿。
